...
#search.printResults(terms, results)
print('total: ' + search.total(results))
search.compileResults('results', terms, results)
#search.writeSpreadsheets('spreadsheets', terms, results)
#search.getTotalSpreadsheet('spreadsheets', results)
...
def writeToCSV(root, lines, file):
    ...
    with open(root + '\\' + file, 'w') as csvfile:
        filewriter = csv.writer(csvfile)
        filewriter.writerows(lines)
...
empty = set([])
pmaInfoList('spreadsheets', 'none', empty)
